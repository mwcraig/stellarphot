...
+ The image viewers in the seeing-profile and comparison-star widgets now use
  ...
  marking the target is now drawn as a true circle of the requested size
...
+ ``mag_cal_error`` from ``transform_to_catalog()`` now adds the image's
  ``fit_excess_scatter`` in quadrature, so it reflects the scatter actually
  observed about the transform rather than only the quoted errors. [#698]
...
+ The comparison viewer's "Click closer to a star" message is now visible: it
  ...
  a star. The circle marking the target no longer leaks into the aperture
  file generated by the viewer. [#584]
+ ``CatalogData(..., no_catalog_error=True)`` now fills in a NaN
  ``mag_error`` column even when ``colname_map`` is not provided (e.g. the
  input table already uses the standard column names); previously the flag
  was silently ignored in that case and a missing ``mag_error`` column
  still raised. [#649]
+ ``PanStarrs1ToJohnsonCousinsMixin.__call__`` now looks up the ``gp1`` and
